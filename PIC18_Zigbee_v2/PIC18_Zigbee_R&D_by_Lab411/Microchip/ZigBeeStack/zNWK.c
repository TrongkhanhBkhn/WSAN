...
#include "Console.h"
// these preprocessor is added by lab411 @dat_a3cbq91
#ifdef USE_ZCP_DEBUG
#define ZCP_DEBUG
#define ZCP_PRINTOUT
#endif
...
HandleRouteRequest:
                                    {
                                        ...
                                        BYTE                    originalCost;
                                        ROUTE_DISCOVERY_ENTRY *currentRouteDiscoveryEntry;
                                            ...
                                            (params.MCPS_DATA_indication.DstAddr.ShortAddr.Val == 0xFFFF))
                                        {
                                            ...
                                            if (rdIndex < ROUTE_DISCOVERY_TABLE_SIZE)
                                            {
                                                ...
                                                {
#ifdef ZCP_PRINTOUT									//3 commands in the following is added by lab411 @dat_a3cbq91
                                                    ConsolePutROMString((ROM char *)"\r\nrreq.pathCost = "); PrintChar(rreq.pathCost);
                                                    ConsolePutROMString((ROM char *)"\r\nrouteDiscoveryTablePointer[rdIndex]->forwardCost = ");
													PrintChar(routeDiscoveryTablePointer[rdIndex]->forwardCost); ConsolePutROMString((ROM char *)"\r\n");
#endif
                                                        ...
                                                        (currentNeighborRecord.deviceInfo.bits.deviceType == DEVICE_ZIGBEE_END_DEVICE)*/  )) 
                                                    {
                                                        ...
                                                        if (ZigBeeReady())
                                                        {
// -------------------------Added by DatBKHN 18/05/2011 (lab411 @dat_a3cbq91)-------------------------
#ifdef ZCP_PRINTOUT
                                                            ConsolePutROMString((ROM char *)"\r\n----------------------ROUTE DISCOVERY TABLE-------------------\r\n");
                                                            //ConsolePutROMString((ROM char *)"routingTableIndex\tScrAddr\tSenderAddr\tforwardRREQ\t");
                                                            //ConsolePutROMString((ROM char *)"routeRequestID\tforwardCost\tresidualCost\r\n");
                                                            ConsolePutROMString((ROM char *)"rtIndex\tSrc\tSender\tfwdRREQ\trRID\tfwdCost\tprevCost\tresCost\r\n");
                                                            for (i = 0; i < ROUTE_DISCOVERY_TABLE_SIZE; i++) {
                                                                ConsolePutROMString((ROM char *)"------------------------------------------------------\r\n");
                                                                PrintChar(routeDiscoveryTablePointer[i]->routingTableIndex);
                                                                ConsolePutROMString((ROM char *)"\t");
                                                                PrintChar(routeDiscoveryTablePointer[i]->srcAddress.v[1]);
                                                                PrintChar(routeDiscoveryTablePointer[i]->srcAddress.v[0]);
                                                                ConsolePutROMString((ROM char *)"\t");
                                                                PrintChar(routeDiscoveryTablePointer[i]->senderAddress.v[1]);
                                                                PrintChar(routeDiscoveryTablePointer[i]->senderAddress.v[0]);
                                                                ConsolePutROMString((ROM char *)"\t");
                                                                PrintChar(routeDiscoveryTablePointer[i]->forwardRREQ);
                                                                ConsolePutROMString((ROM char *)"\t");
                                                                PrintChar(routeDiscoveryTablePointer[i]->routeRequestID);
                                                                ConsolePutROMString((ROM char *)"\t");
                                                                PrintChar(routeDiscoveryTablePointer[i]->forwardCost);
                                                                ConsolePutROMString((ROM char *)"\t");
                                                                PrintChar(routeDiscoveryTablePointer[i]->previousCost);
                                                                ConsolePutROMString((ROM char *)"\t");
                                                                PrintChar(routeDiscoveryTablePointer[i]->residualCost);
                                                                ConsolePutROMString((ROM char *)"\r\n------------------------------------------------------\r\n");
                                                            }
                                                            ConsolePutROMString((ROM char *)"\r\nLua chon tuyen co rdIndex: ");
                                                            PrintChar(rdIndex);
                                                            ConsolePutROMString((ROM char *)"\r\nLua chon tuyen co rtIndex: ");
                                                            PrintChar(routeDiscoveryTablePointer[rdIndex]->routingTableIndex);
                                                            ConsolePutROMString((ROM char *)"\r\n");
#endif
// -------------------------END by DatBKHN 18/05/2011-------------------------
...
//            We handle everything with the NWK LEAVE command.
            ...
            case MLME_DISASSOCIATE_indication:
            	#ifdef ZCP_PRINTOUT
                ConsolePutROMString((ROM char *)"MLME_DISASSOCIATE_indication Result: ");//Printf -> consolePutROMstring by lab411 @dat_a3cbq91
                ConsolePutROMString((ROM char *)"\r\nDevice Address: ");
                ...
                if( params.MLME_DISASSOCIATE_indication.DisassociateReason == COORDINATOR_FORCED_LEAVE )
                {
                    ...
                    ConsolePutROMString((ROM char *)"\r\nMAC PIB: ");	//function put string that modified by lab411  @dat_a3cbq91
                    ConsolePutROMString((ROM char *)"\r\nMAC PANID: ");
            ...
            case MLME_BEACON_NOTIFY_indication:
			...
			#ifdef ZCP_PRINTOUT
                ConsolePutROMString((ROM char *)"\r\nBeacon Notify Indication Result:");	//function put string that modified by lab411  @dat_a3cbq91
                ConsolePutROMString((ROM char *)"\r\nCoordAddrMode: ");
                ...
                ConsolePutROMString((ROM char *)"\r\nLogical Channel: ");	//function put string that modified by lab411  @dat_a3cbq91
                ConsolePutROMString((ROM char *)"\r\nLogical Channel: ");	//function put string that modified by lab411  @dat_a3cbq91
                ConsolePutROMString((ROM char *)"\r\nLogical Channel: ");	//function put string that modified by lab411  @dat_a3cbq91
...
ResetNeighborTableFields:
                            ...
                            currentNeighborRecord.deviceInfo.bits.RxOnWhenIdle      = 1;//Transceiver ON trong che do ngu? by lab411
...
NetworkStartupFailure:
					...
					#ifdef ZCP_PRINTOUT
                        ConsolePutROMString((ROM char *)"Scan Confirm Result(Energy Scan):");	//function put string that modified by lab411  @dat_a3cbq91
                        ConsolePutROMString((ROM char *)"\r\nUnscanned Channels: ");
                        ...
                        #if defined(I_SUPPORT_FREQUENCY_AGILITY) && defined(I_AM_NWK_MANAGER)
                            ConsolePutROMString((ROM char *)"\r\nEnergy Table: ");	//function put string that modified by lab411  @dat_a3cbq91
                            for(i = 0; i < 16; i++)
                            {
...
StartActiveScan:
                    ...
                    if (params.MLME_SCAN_confirm.ScanType == MAC_SCAN_ACTIVE_SCAN)
                    {
                        ...
                        if (params.MLME_SCAN_confirm.status)
                        {
                            ...
                            for(i = 0; i < params.MLME_SCAN_confirm.ResultListSize; i++)
                            {
                                ConsolePutROMString((ROM char *)"PAN ");	//function put string that modified by lab411  @dat_a3cbq91
                                PrintChar(i);
                                ConsolePutROMString((ROM char *)": ");
                                ...
                                ConsolePutROMString((ROM char *)" ");	//function put string that modified by lab411  @dat_a3cbq91
...
RejoinNetworks:                            
                    ...
                    else
                    {
                        ...
                        ZigBeeStatus.flags.bits.bTryingToJoinNetwork = 0;
					#ifdef ZCP_PRINTOUT
                        ConsolePutROMString((ROM char *)"\r\nScan Confirm Result(Orphan Scan):");	//function put string that modified by lab411  @dat_a3cbq91
                        ConsolePutROMString((ROM char *)"\r\nUnscanned Channels: ");
                        PrintChar(params.MLME_SCAN_confirm.UnscannedChannels.v[3]);
                        PrintChar(params.MLME_SCAN_confirm.UnscannedChannels.v[2]);
                        PrintChar(params.MLME_SCAN_confirm.UnscannedChannels.v[1]);
            ...
            case MLME_COMM_STATUS_indication:
                #if defined(I_AM_COORDINATOR) || defined(I_AM_ROUTER)
					#ifdef ZCP_PRINTOUT
                    ConsolePutROMString((ROM char *)"\r\nMLME_COMM_STATUS_indication Result: ");	//function put string that modified by lab411  @dat_a3cbq91
                    ConsolePutROMString((ROM char *)"\r\nPANID: ");
...
MESSAGE_ROUTING_STATUS InitiateRouteDiscovery( BOOL fromUpperLayers, BYTE discoverRoute )
{
    ...
    if (!(ptr = routeDiscoveryTablePointer[rdIndex]->forwardRREQ = SRAMalloc(sizeof_ROUTE_REQUEST_COMMAND+8)))
    {
        ...
        SRAMfree( nwkStatus.routingMessages[messageIndex]->dataPtr );
        nfree( nwkStatus.routingMessages[messageIndex] );
        //ConsolePutROMString((ROM char *)"ENwk_4");	//function put string that modified by lab411  @dat_a3cbq91
        return ROUTE_FAILURE_NO_CAPACITY;
    }
...
void Prepare_MCPS_DATA_request( WORD macDestAddressVal, BYTE *msduHandle )
{
    ...
    params.MCPS_DATA_request.msduHandle = *msduHandle = MLME_GET_macDSN();// dung de giam sat MSDU - the comment add by lab411
...
void VariableAndTableInitialization( BOOL force )
{
    ...
    nwkStatus.flags.bits.bNRejoin = i;
        ...
        NIB.nwkSymLink                          = DEFAULT_nwkSymLink;//FALSE --> nonsymetric routing - the comment added by lab411
        NIB.nwkCapabilityInformation            = DEFAULT_nwkCapabilityInformation;
        NIB.nwkUseTreeAddrAlloc                 = DEFAULT_nwkUseTreeAddrAlloc;
        NIB.nwkUseTreeRouting                   = DEFAULT_nwkUseTreeRouting;//TRUE
    #endif
