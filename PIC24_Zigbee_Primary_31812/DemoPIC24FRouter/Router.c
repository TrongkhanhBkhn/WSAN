...
//for sleep mode @by Hoang Anh Son
    ...
    BYTE SLEEP_DURATION  = 48;
    BYTE WAKE_DURATION = 6;
...
int main(void)
{
    ...
    orphanTries = 3;
       ...
       CYCLE_SENDING = 1440 * ONE_MINUTE_VALUE;//chu k? g?i d? li?u ??nh k? là 1 ngày 1 l?n
                ...
                switch (params.APSDE_DATA_indication.DstEndpoint)
                {
                    case WSAN_Endpoint:
                    {
                        switch (params.APSDE_DATA_indication.ClusterId.Val)
                        {
                            ...
                            #if defined(USE_SHT10)
                                // send position to Router Emboard
                                // TrongkhanhBKHN
                                case POSITION_REQUEST_CLUSTER:
                                {
                                    data = APLGet();
                                    LoadSHT10();
                                    Send_HTE_ToRouterEmboard(POSITION_RESPONSE_CLUSTER);
                                    #if defined(USE_DEBUG)
            ...
            else
            {
                if(!ZigBeeStatus.flags.bits.bHasBackgroundTasks)
                {
                    ...
                    // ************************************************************************
                        ...
                        if(ZigBeeReady())
                        {
                            if(WSANFlags.bits.HTE_Data_Ready)
                            {
                            ...
                            if(WSANFlags.bits.HTE_Data_Ready5M){
                                WSANFlags.bits.HTE_Data_Ready5M = CLEAR;                              
                                Send_HTE_ToRouterEmboard(HTE_AUTO_SEND_CLUSTER);
                                #if defined(USE_DEBUG)
                                     printf("Sending warning data to RE \r\n");
                                #endif
                            }
                        }
...
void ProcessNONZigBeeTasks(void)
{
    ...
    // *********************************************************************
        ...
        if(WSANFlags.bits.EnableGetDataHTE5M){
            ...
            LoadSHT10();
            #if defined(USE_DEBUG)
            printf("Du lieu kiem tra dinh ky \r\n")
                printf("Temperature (hex): ");
                PrintWord(temperature);
                printf("\r\n");
                ...
                {
                    WSANFlags.bits.HTE_Data_Ready5M = SET;
                }
        }
        #if defined(USE_SAVE_ENERGY_MODE)
...
void _ISR __attribute__((interrupt, auto_psv)) _T3Interrupt(void)
{
    ...
    if (WSANFlags.bits.DeviceMode == ActiveMode)//Devicemode = 1 --> Save Energy.
    {
        ...
        if(TimerCount%CYCLE_SENDING_5M == 0){
            #if defined(USE_SHT10)
